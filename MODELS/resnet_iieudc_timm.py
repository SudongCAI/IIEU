...
class IIEUBDC(nn.Module):
    def __init__(self, inplanes, planes, rate_reduct=8, spatial=8, g=1):
        ...
        # r * sigma
        self.bound = Parameter(0.01 * torch.ones(1, 1, planes)) #  
...
################
# only global version (ver8 revised: softmax comb, concat x and res, dense-dnl-dense)
# bias=False: 80.254% with resnet50 backbone
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.scaler = DynamicScaler(inplanes=planes * self.expansion, planes=planes * self.expansion, 
                               rate_reduct=8) # rate_reduct default: 8
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        # init
        #self.init_weights(zero_init_last=False)
...
@register_model
def iieudc_resnet14(pretrained=False, **kwargs):
    """
    """
    model_args = dict(block=Bottleneck, layers=[1, 1, 1, 1],  **kwargs)
    return build_model_with_cfg(
        ResNet, 'iieudc_resnet14', default_cfg=default_cfg['resnet14'],
...
@register_model
def iieudc_resnet26(pretrained=False, **kwargs):
    """
    """
    model_args = dict(block=Bottleneck, layers=[2, 2, 2, 2],  **kwargs)
    return build_model_with_cfg(
        ResNet, 'iieudc_resnet26', default_cfg=default_cfg['resnet26'],
        pretrained=pretrained, **model_args)


@register_model
def iieudc_resnet50(pretrained=False, **kwargs):
    """
    """
    model_args = dict(block=Bottleneck, layers=[3, 4, 6, 3],  **kwargs)
    return build_model_with_cfg(
        ResNet, 'iieudc_resnet50', default_cfg=default_cfg['resnet50'],
        pretrained=pretrained, **model_args)
